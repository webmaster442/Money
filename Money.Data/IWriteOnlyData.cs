...
public interface IWriteOnlyData
{
    ...
    Task<(int createdCategory, int createdEntry)> ImportAsync(IEnumerable<DataRowExcel> rows);
    Task<(int createdCategory, int createdEntry)> ImportBackupAsync(IEnumerable<DataRowBackup> rows);
    Task<int> ClearDb();
}
